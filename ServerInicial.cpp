  ...
  while (active) {
    ...
    if (lector.open(getPath(false) + titulo + ".txt") == 0) {
      if (ceroPriv->cambiarArchivoActual(getPath(true), tituloNumero + 1)) {
        ...
        //Aqui Token // Llave 1
        std::string tolkien = cifrado.encryptMessage(token.c_str(), "/home/manuel.arroyoportilla/In-secure/key.pem");
        if (procesador->abrir(tolkien, shaFile, getPath(false), titulo)) {
          while (lector.read()) {
            std::string chunk = lector.getText();
            //Aqui Chunk // Llave 2 
            std::string chunkie = cifrado.encryptMessage(chunk.c_str(), "/home/manuel.arroyoportilla/In-secure/key2.pem");
